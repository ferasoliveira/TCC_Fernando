...
void setup() {
  ...
  Serial.println("[SISTEMA] Ponte UART Serial c/ ESP em A4/A5 ativa.");
  ...
  if (espSerial.available()) {
    ...
    if(cmd.startsWith("TEST_ENC:")) {
      ...
      if(c1 > 0 && c2 > c1 && c3 > c2) {
        ...
        Serial.printf("[PARSER] Moto Alvo: %d | Passos Requeridos: %d | Sentido: %d\n", m, steps, dir);
        ...
        Serial.printf("[SENSOR] Consultando encoder do motor via TCA canal %d...\n", ch);
        long startDeg = readEncoderDegrees(ch);
        Serial.printf("[SENSOR] Angulo inicial capturado pre-acionamento mecanico: %ld graus\n", startDeg);
        ...
        Serial.printf("[SENSOR] Terminologia Pós: %ld graus | Variacao (Delta) = %ld graus\n", endDeg, delta);
        
        // Devolve ao ESP
        // Formato: RES_ENC:M,DELTA
        Serial.printf("[TX] Remetendo o payload (RES_ENC:%d,%ld) de forma assincrona p/ ESP32 via UART.\n", m, delta);
