...
void setup() {
  ...
  Serial1.begin(115200, SERIAL_8N1, RX_PIN, TX_PIN); // USART1
  Serial.printf("[SISTEMA] Serial1 atrelada aos pinos RX=%d TX=%d a 115200 baud bps.\n", RX_PIN, TX_PIN);
  ...
  while (Serial1.available()) {
    ...
    if(resp.length() > 0) {
      ...
      if(resp == "PING_FROM_ARDUINO") {
         Serial.println("[LINK] Pacote do Arduino validado como PING. Emitindo ACK_FROM_ESP na porta Serial1.");
         Serial1.println("ACK_FROM_ESP");
      } else {
