...
Já no core 0 (Núcleo de Serviços Assíncronos e Conectividade), o núcleo executa as rotinas relacionadas às pilhas de protocolo de rede sem fio, que demandam processamento assíncrono e imprevisível. A tarefa \texttt{Task\_WiFi} (prioridade 3) gerencia a estabilidade da interface de rede sem fio nos modos simultâneos \textbf{Station (STA)} e \textbf{Access Point (AP)}. 
...
\subsection{Firmware Dedicado no Arduino Uno}
...
\label{sec:validacao}
\textcolor{blue}{[ REVISADO ATÉ AQUI - Vou melhorar esse paragrafo com um procedimento melhor e mais direto, incluindo 4 etapas de validação: Simulação da interação de Hardwares, Simulação da robótica (usado o webots), teste individual do protótipo, para validar hardwares isolados e em conjuto (sem ativação das juntas), e testes de execução mecânica (com ativação das juntas + Encoders)]}
